...
//---
   getDayOfWeek();
   showDashboard();
   if(CheckAllowedTradingDay() == true) {
      ...
      if(sell_entry_signal != EMPTY_VALUE && sell_entry_signal < 2147483647) {
         if(orders_open == false) {
            PlaceSellOrder();
            //takeScreenshotOfTrade("SELL_ENTRY");
         ...
         if(orders_open == false) {
            PlaceBuyOrder();
            //takeScreenshotOfTrade("BUY_ENTRY");
...
void CloseAllOpenTrades() {
   ...
   orders_open = false;
   //takeScreenshotOfTrade("TRADE_EXIT");
}
...
void showDashboard() {
      ...
      DisplayCount=0;
      AddDashBoardMessage("---------------------------");
      AddDashBoardMessage("Nigel! GBPJPY EA Dashboard");
      AddDashBoardMessage("---------------------------");
      AddDashBoardMessage("");      
      AddDashBoardMessage("Days running: " + days_running);
      AddDashBoardMessage("Day of week: " + day_of_week + " " +Hour() + ":" +Minute());
      AddDashBoardMessage("Trading Status: " + tradingHoursStatus);
...
void takeScreenshotOfTrade(string action) {
   screenshotCounter++;
   string name = "screenshots\\screenshots_"+Symbol()+"_"+string(screenshotCounter)+"_"+action+".gif";
   WindowScreenShot(name,1280,720);
   Print("ScreenShot saved to dir: " + name);
}
