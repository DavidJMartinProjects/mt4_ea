...
void OnTick()
  {
     ...
     buy_entry_signal = getEntrySignal(1, 1);
     sell_entry_signal = getEntrySignal(2, 1);
     
     if(buy != EMPTY_VALUE && buy < 2147483647 && fast_uptrend < 2147483647 && slow_uptrend < 2147483647){
         // close open shorts         
         Print("======> BUY Order: buy_entry_signal: " + buy_entry_signal + "  sell_entry_signal : " + sell_entry_signal +"<======");
            if(buy_entry_signal != EMPTY_VALUE &&  buy_entry_signal != 0) {
            // Open Buy Order
            Print("++++++> BUY Order: Valid Entry Detected. <++++++");
         ...
         // close open buys
         Print("======> SELL Order: sell_entry_signal : " + sell_entry_signal + " +  buy_entry_signal: " + buy_entry_signal  +"<======");
         if(sell_entry_signal != EMPTY_VALUE && sell_entry_signal != 0) {     
            //Open Sell Order   
            Print("++++++> SELL Order: Valid Entry Detected. <++++++");      
